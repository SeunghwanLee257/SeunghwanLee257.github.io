...
|------|----------|------|------|----------|---------------|
| 1 | 벤치마킹 버튼 깜빡임 | 새로고침 시 버튼이 나타났다가 사라지는 현상 | - CSS transition이 display/visibility 변경에 적용됨<br>- JavaScript 실행 순서 문제<br>- 브라우저 캐시<br>- 미디어 쿼리 CSS 규칙이 기본 CSS를 덮어씀 | 1. CSS에서 `opacity: 0` 설정<br>2. `is-positioned` 클래스 추가<br>3. 인라인 스타일 추가<br>4. MutationObserver 사용<br>5. Head에 즉시 실행 스크립트 추가<br>6. 버튼을 HTML에서 제거하고 JavaScript로 동적 생성 | - 버튼을 HTML에서 완전히 제거<br>- JavaScript에서 `window.load` 이벤트 후 동적 생성<br>- `is-ready` 클래스로 표시 제어<br>- CSS transition을 `background, color`에만 적용 |
